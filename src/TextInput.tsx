...
import { LabelType, LanguageItemState, LanguageType, currentLanguage, currentSubmenu, languages, setSubMenu } from "./languageSlice";
import { IconButton, InputAdornment, Paper, TextField } from '@mui/material';
import { Close, Language } from '@mui/icons-material';
import getUnicodeFlagIcon from 'country-flag-icons/unicode'

function TextInput({ inputId, labels } : { inputId: string, labels: LabelType }) {
    ...
    const changeTranslateValue = (selectedLang: LanguageType) => (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const blurTranslateValue = (selectedLang: LanguageType) => (event: React.FocusEvent<HTMLInputElement>) => {
    ...
    const handleIgnoreMouseEvent = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <>
        ...
        <TextField  onFocus={onFocusCurrentValue} 
        onChange={changeCurrentValue} onBlur={onBlurCurrentValue} 
        label={ `${labels[current]} ${lang == current ? '' : `(🌐 ${getUnicodeFlagIcon(lang == 'en' ? 'gb' : lang)})`}` } 
        value={text}
        size='small'        
        slotProps={{
            input: {
        endAdornment:
            <InputAdornment position="end">
              <IconButton
                aria-label={
                  currentSub == inputId ? 'hide the password' : 'display the password'
                }
        ...
        ></TextField>
           ...
           { currentSub == inputId ? 
            <Paper className="lang-box" elevation={3}>
                { newLang != null ? 
                <div>
                <TextField inputRef={newLangInputRef} size="small" value={data.values.get(newLang)} label={labels[newLang]} 
                ...
                { orderedKeys.filter((item) => item != current && item != newLang).map((item) => 
                    <div>
                        <TextField size="small" value={data.values.get(item)} label={labels[item]} 
                ...
                <div> 
                    { languages.filter((item) => (orderedKeys.indexOf(item) < 0) && item != newLang).map((item) => 
                        <><IconButton onClick={addLanguage(item)}>{getUnicodeFlagIcon(item == 'en' ? 'gb' : item)}</IconButton></> ) 
                    }
                </div>
            </Paper> : <></>
}
